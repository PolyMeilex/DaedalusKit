...
#[derive(Default)]
struct AutogeneratedSymbols {
    symbols: Vec<Symbol>,
}

impl AutogeneratedSymbols {
    pub fn len(&self) -> usize {
        self.symbols.len()
    }

    pub fn push_string(&mut self, value: ZString) {
        let symbol_id = 10000 + self.symbols.len();
...
struct Stage2 {
    symbol_table: HashMap<ZString, u32>,
    dat: DatBuilder,
    autogenerated_symbols: AutogeneratedSymbols,
}
...
impl Stage2 {
    pub fn new(stage1: Stage1) -> Self {
        Self {
            ...
            dat: DatBuilder::new(),
            autogenerated_symbols: AutogeneratedSymbols::default(),
        }
    }
    ...
    fn handle_item(&mut self, files: &Files, file_id: FileId, item: &daedalus_parser::Item) {
        match item {
            ...
            daedalus_parser::Item::Instance(instance) => {
                ...
                let this = *self.symbol_table.get(&ident).unwrap();

                self.dat.gen_instance(ident, span, address, *parent);

                let humans_mds =
                    (self.symbol_table.len() + self.autogenerated_symbols.len()) as u32;
                self.autogenerated_symbols
                    .push_string(ZString::from("HUMANS.MDS"));

                let hum_body_naked0 =
                    (self.symbol_table.len() + self.autogenerated_symbols.len()) as u32;
                self.autogenerated_symbols
                    .push_string(ZString::from("hum_body_Naked0"));

                let hum_head_pony =
                    (self.symbol_table.len() + self.autogenerated_symbols.len()) as u32;
                self.autogenerated_symbols
                    .push_string(ZString::from("Hum_Head_Pony"));
    ...
    pub fn run(
        ...
        for symbol in self.autogenerated_symbols.symbols {
            self.dat.push_symbol(symbol);
        }
